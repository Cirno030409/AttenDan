...
def showgui_main():  # GUIを表示
    ...
    while True:
        ...
        # 生徒登録画面のボタン
            ...
            if values["-st_name-"] == "" or values["-st_age-"] == "" or values["-st_gender-"] == "" or values["-st_parentsname-"] == "" or values["-st_mail_address-"] == "":
                sg.PopupOK("登録するには，生徒の情報をすべて入力する必要があります。", title="生徒登録エラー", keep_on_top=True, modal=True)
                continue
            register_student(
                values["-st_name-"],
                values["-st_age-"],
...
def register_student(
    ...
    if (id := wait_card_popup("登録する生徒のカードをタッチしてください")) == -1:
        ...
        data = {"name" : name, "age" : age, "gender" : gender, "parent_name" : parent_name, "mail_address" : mail_address, "id" : id, "attendance" : "退席"}
        if db.add_student_to_database(data) == -1:
            sg.popup_ok("生徒の登録に失敗しました。詳細はシステム出力を参照してください。", title="エラー", keep_on_top=True, modal=True)
        else:
